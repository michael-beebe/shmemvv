 ...
 *
 * This function prints out the usage information and available options for the
 * OpenSHMEM verification/validation test suite.
 */
void display_help() {
  printf("\nThis program is a verification/validation test suite for OpenSHMEM "
         "implementations.\n");
 ...
 * options in the provided test_options structure.
 *
...
bool parse_opts(int argc, char *argv[], test_options *opts) {
  ...
  int option_index = 0;
  int c;
  while ((c = getopt_long(argc, argv, "abcdefghijklmn", long_options,
                          &option_index)) != -1) {
    switch (c) {
    case 'a':
  ...
  /* If no specific tests are selected and --all is not specified, enable all
   * tests */
  if (!(opts->test_setup || opts->test_threads || opts->test_mem ||
        opts->test_teams || opts->test_ctx || opts->test_remote ||
        opts->test_atomics || opts->test_signaling || opts->test_collectives ||
        opts->test_pt2pt_synch || opts->test_mem_ordering ||
 ...
 *
 * This function prints out a header indicating the start of a specific test
 * category.
 *
 ...
 * tested, including the library name, version, and the number of PEs
 * (Processing Elements).
 *
 ...
 *
 * This function checks if a given OpenSHMEM routine exists in the currently
 * loaded library.
 *
 ...
 *
 * This function prints a warning message indicating that a specific OpenSHMEM
 * routine is not available in the library being tested.
 *
...
void display_not_found_warning(const char *routine_name, bool required) {
  if (required) {
    fprintf(stderr,
            RED_COLOR "NOT FOUND - required to continue" RESET_COLOR ": %s\n",
            routine_name);
  } else {
