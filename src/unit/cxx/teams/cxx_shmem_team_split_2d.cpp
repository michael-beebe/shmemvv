/**
 * @file cxx_shmem_team_split_2d.cpp
...
bool test_shmem_team_split_2d(void) {
  ...
  log_info("Using xfactor=2");

  shmem_team_split_2d(SHMEM_TEAM_WORLD, 2, NULL, 0, &team_x, NULL, 0, &team_y);

  log_info("Team split successful, getting number of PEs in each team");
...
int main(int argc, char *argv[]) {
  ...
  if (mype == 0) {
    display_test_result("CXX shmem_team_split_2d()", result, false);
  }
