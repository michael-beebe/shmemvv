/**
 * @file cxx_shmem_atomic_swap_nbi.cpp
...
#define TEST_CXX_SHMEM_ATOMIC_SWAP_NBI(TYPE, TYPENAME)                         \
  ({                                                                           \
    ...
    *dest = value;                                                             \
    ...
    shmem_barrier_all();                                                       \
    log_info("executing atomic swap_nbi: dest = %p, new_val = %d",             \
    ...
    if (!success)                                                              \
      log_fail("atomic swap_nbi on %s did not produce expected values: "       \
...
int main(int argc, char *argv[]) {
  ...
  if (shmem_my_pe() == 0) {
    display_test_result("CXX shmem_atomic_swap_nbi()", result, false);
  }
