/**
 * @file cxx_shmem_atomic_fetch_and_nbi.cpp
...
#define TEST_CXX_SHMEM_ATOMIC_FETCH_AND_NBI(TYPE, TYPENAME)                    \
  ({                                                                           \
    ...
    *dest = value;                                                             \
    ...
    shmem_barrier_all();                                                       \
    log_info("executing atomic fetch_and_nbi: dest = %p, and_val = %d",        \
    ...
    if (!success)                                                              \
      log_fail("atomic fetch_and_nbi on %s did not produce expected values: "  \
...
int main(int argc, char *argv[]) {
  ...
  if (shmem_my_pe() == 0) {
    display_test_result("CXX shmem_atomic_fetch_and_nbi()", result, false);
  }
