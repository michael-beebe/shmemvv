...
bool test_shmem_team_my_pe(void) {
  ...
  log_info("World PE %d of %d attempting to split SHMEM_TEAM_WORLD", world_mype,
           world_npes);
  log_info("Using stride=1, start=0 and all PEs");
  int ret = shmem_team_split_strided(SHMEM_TEAM_WORLD, 0, 1, world_npes, NULL,
                                     0, &team);
  ...
  bool valid = false;
  if (my_pe == -1) {
    log_fail("shmem_team_my_pe() returned -1, indicating PE %d is not in team",
             world_mype);
  } else if (my_pe < 0 || my_pe >= team_npes) {
    log_fail("shmem_team_my_pe() returned invalid PE %d (valid range: 0-%d)",
             my_pe, team_npes - 1);
  } else {
