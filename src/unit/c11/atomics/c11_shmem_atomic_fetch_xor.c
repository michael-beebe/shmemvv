...
#define TEST_C11_SHMEM_ATOMIC_FETCH_XOR(TYPE)                                  \
  ({                                                                           \
    ...
    *dest = value;                                                             \
    ...
    TYPE fetched = shmem_atomic_fetch_xor(dest, xor_value, (mype + 1) % npes); \
    shmem_barrier_all();                                                       \
  ...
  ({                                                                           \
    ...
    *dest = value;                                                             \
    ...
    log_info("executing atomic fetch-xor with context: dest = %p, value = %d", \
             (void *)dest, (int)xor_value);                                    \
    TYPE fetched =                                                             \
        shmem_atomic_fetch_xor(ctx, dest, xor_value, (mype + 1) % npes);       \
    shmem_ctx_quiet(ctx);                                                      \
    shmem_barrier_all();                                                       \
    success = (fetched == value && *dest == (value ^ xor_value));              \
    if (!success)                                                              \
...
int main(int argc, char *argv[]) {
  ...
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_XOR(unsigned int);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_XOR(unsigned long);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_XOR(unsigned long long);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_XOR(int32_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_XOR(int64_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_XOR(uint32_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_XOR(uint64_t);

  if (shmem_my_pe() == 0) {
    display_test_result("C11 shmem_atomic_fetch_xor with ctx", result_ctx,
                        false);
  }
