...
#define TEST_C11_SHMEM_ATOMIC_FETCH_INC_NBI(TYPE)                              \
  ({                                                                           \
    ...
    *dest = value + mype;                                                             \
    ...
    shmem_atomic_fetch_inc_nbi(&fetch, dest, fetch_pe);                        \
    shmem_quiet();                                                             \
    shmem_barrier_all();                                                       \
    success = (fetch == value + fetch_pe && *dest == value + mype + 1);        \
    if (!success)                                                              \
      log_fail("atomic fetch inc (nbi) on %s did not produce values: "         \
  ...
  ({                                                                           \
    ...
    *dest = value + mype;                                                      \
    ...
    if (!success)                                                              \
      ...
      log_info("atomic fetch inc (nbi) with ctx on a %s at %p produced "       \
