...
#define TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(TYPE)                          \
  ({                                                                           \
    log_routine("shmem_atomic_fetch_inc_nbi(ctx, " #TYPE ")");                 \
    bool success = true;                                                       \
    static TYPE *dest;                                                         \
    static TYPE fetch;                                                         \
    dest = (TYPE *)shmem_malloc(sizeof(TYPE));                                 \
    ...
    *dest = value;                                                             \
    ...
    log_info("executing atomic fetch inc nbi with context: dest = %p, "        \
             ...
             (void *)dest, (char)*dest);                                       \
    int mype = shmem_my_pe();                                                  \
    shmem_atomic_fetch_inc_nbi(ctx, &fetch, dest, mype);                       \
    shmem_ctx_quiet(ctx);                                                      \
    shmem_barrier_all();                                                       \
    success = (fetch == value && *dest == value + 1);                          \
    if (!success)                                                              \
      log_fail("atomic fetch inc nbi with context on %s did not produce "      \
               "expected ret = %d, *dest = %d, got instead ret = %d, "         \
               "*dest = %d",                                                   \
               #TYPE, (char)value, (char)(value + 1), (char)fetch,             \
               (char)*dest);                                                   \
    else                                                                       \
...
int main(int argc, char *argv[]) {
  ...
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(int);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(long);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(long long);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(unsigned int);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(unsigned long);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(unsigned long long);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(int32_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(int64_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(uint32_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(uint64_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(size_t);
  result_ctx &= TEST_C11_CTX_SHMEM_ATOMIC_FETCH_INC_NBI(ptrdiff_t);

  if (shmem_my_pe() == 0) {
    display_test_result("C11 shmem_atomic_fetch_inc_nbi with ctx", result_ctx,
                        false);
  }
