...
#define TEST_C11_SHMEM_ATOMIC_FETCH_AND_NBI(TYPE)                              \
  ({                                                                           \
    ...
    *dest = value;                                                             \
    ...
    log_info("executing atomic fetch and (nbi): dest = %p, and_val = %d", (void *)dest, (char)and_val);               \
    shmem_atomic_fetch_and_nbi(&fetch, dest, and_val, mype);                     \
    shmem_quiet();                                                             \
    shmem_barrier_all();                                                       \
    success = (fetch == value && *dest == (value & and_val));                   \
    if (!success)                                                              \
      log_fail("atomic fetch and (nbi) on %s did not produce expected value = %d, ret = "   \
               "%d, got "                                                      \
