...
bool test_shmem_team_sync(void) {
  ...
  if (shared_counter != shmem_n_pes()) {
    log_fail("Shared counter validation failed: expected %d, got %ld", shmem_n_pes(), shared_counter);
    if (shared_counter < shmem_n_pes()) {
      log_fail("Team synchronization may have failed to propagate all atomic operations");
    } else {
      log_fail("Either team split created incorrect number of teams or team sync duplicated operations");
    }
    success = false;
  } else {
    log_info("Shared counter validation successful: value matches expected %d", shmem_n_pes());
  }
