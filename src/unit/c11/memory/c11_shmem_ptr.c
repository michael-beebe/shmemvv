  ...
  *ptr = mype;
  ...
  for (int pe = 0; pe < npes; ++pe) {
    ...
    if (remote_ptr != NULL) {
      ...
      if (remote_val != pe) {
        log_fail("remote ptr read (%p) produced unexpected value (expected %d, got %d)", (void*)remote_ptr, pe, remote_val);
        test_passed = false;
      } else {
      ...
      log_warn("ptr was null, but was ptr to remote pe. your openshmem impl may not support shmem_ptr");
    }
  }
